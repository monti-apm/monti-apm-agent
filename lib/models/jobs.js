...
let jobLogger = require('debug')('kadira:job-monitoring');
export { jobLogger };
...
JobsModel.prototype.trackNewJob = function (jobName, count = 1) {
  const timestamp = Ntp._now();
  const dateId = this._getDateId(timestamp);

  let jobMetrics = this._getMetrics(dateId, jobName);
  jobMetrics.added += count;

  if (!this.jobMetricsByMinute[dateId].startTime) {
    this.jobMetricsByMinute[dateId].startTime = timestamp;
  }
};
...
JobsModel.prototype.trackPendingJobs = function (jobName, pendingCount) {
  jobLogger(`model - trackPendingJobs (${jobName}) - ${pendingCount}`);
...
JobsModel.prototype.processJob = function (jobTrace, waitTime) {
  ...
  jobLogger(`model - processJob (${jobTrace.name})`);
...
JobsModel.prototype.buildPayload = function () {
  ...
  for (let key of Object.keys(jobMetricsByMinute)) {
    ...
    for (let jobName of Object.keys(jobMetrics.jobs)) {
      JOB_METRICS_FIELDS.forEach(function (field) {
        if (jobMetrics.jobs[jobName][field] === 0) {
          return;
        }
  ...
  jobLogger(`model - collecting job requests (${payload.jobRequests.length})`);
