...
PubsubModel.prototype.buildPayload = function () {
  ...
  for (let dateId of Object.keys(metricsByMinute)) {
    ...
    for (let publication of Object.keys(metricsByMinute[dateId].pubs)) {
      ...
      singlePubMetrics.waitedOn /= singlePubMetrics.subs;
      singlePubMetrics.waitedOn = singlePubMetrics.waitedOn || 0;
...
PubsubModel.prototype.trackWaitTime = function (msg) {
  let timestamp = Ntp._now();
  const { startTime, publication } = this.subscriptions[msg.id];

  let metrics = this._getMetrics(timestamp, publication);

  metrics.waitTime += timestamp - startTime;
};
