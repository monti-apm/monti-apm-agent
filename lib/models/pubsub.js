...
const LIVE_QUERY_POLLING_BUCKET_MS = 1000 * 60;
const LIVE_QUERY_POLLING_MAX_BUCKETS = 16;
const LIVE_QUERY_POLLING_REPORT_INTERVAL_MS = 1000 * 60 * 60;
const LIVE_QUERY_POLLING_DEFAULT_WINDOW_MS = 1000 * 60 * 15;
const LIVE_QUERY_POLLING_DEFAULT_MIN_CYCLES = 3;
const LIVE_QUERY_POLLING_DEFAULT_DOCUMENT_BUDGET = 25000;
const liveQueryPollingState = Symbol('liveQueryPollingState');

export function PubsubModel () {
  this.metricsByMinute = Object.create(null);
  this.subscriptions = Object.create(null);
  this.liveQueryPollingReports = new Map();
...
PubsubModel.prototype._trackNoSub = function (msg) {
  let publication = this.subscriptions[msg.id];
...
PubsubModel.prototype._trackError = function (session, sub, trace) {
  ...
  if (!session) {
    // TODO: it would be better to track this error, but we need to handle
    // if the trace is already built/avoid mutating the trace
    return;
  }
...
PubsubModel.prototype._getSubscriptionInfo = function () {
  ...
  _.each(totalObservers, function (value, publication) {
    avgObserverReuse[publication] =
      cachedObservers[publication] / totalObservers[publication];
  });
...
PubsubModel.prototype.buildPayload = function () {
  ...
  // to the averaging
  for (let dateId of Object.keys(metricsByMinute)) {
    ...
    for (let publication of Object.keys(metricsByMinute[dateId].pubs)) {
      ...
      singlePubMetrics.waitTime /= singlePubMetrics.subs;
      singlePubMetrics.waitTime = singlePubMetrics.waitTime || 0;
...
PubsubModel.prototype.trackDocumentChanges = function (info, op) {
  // It's possible that info to be null
  // Specially when getting changes at the very beginning.
...
PubsubModel.prototype.trackPolledDocuments = function (info, count) {
  // Sometimes info is missing - https://github.com/monti-apm/monti-apm-agent/issues/123
  if (!info) {
    return;
  }
...
PubsubModel.prototype.trackLiveQueryPolling = function (observerDriver, cycle) {
  try {
    this._trackLiveQueryPolling(observerDriver, cycle);
  } catch (error) {
    // Diagnostics must never affect the observer being measured.
    logger('Unable to track repeated live-query polling', error);
  }
};

PubsubModel.prototype._trackLiveQueryPolling = function (observerDriver, cycle) {
  const documentBudget = getLiveQueryPollingOption(
    'liveQueryPollingDocumentBudget',
    LIVE_QUERY_POLLING_DEFAULT_DOCUMENT_BUDGET
  );

  if (documentBudget === 0 || !observerDriver) {
    return;
  }

  const timestamp = cycle?.timestamp ?? Date.now();
  let state = observerDriver[liveQueryPollingState];
  ...
  if (!state) {
    state = { buckets: [] };
    Object.defineProperty(observerDriver, liveQueryPollingState, {
      configurable: true,
      value: state,
    });
  }
  ...
  if (cycle) {
    const bucketStart = timestamp - (timestamp % LIVE_QUERY_POLLING_BUCKET_MS);
    ...
    const duration = getNonNegativeNumber(cycle.duration);
    bucket.polls++;
    bucket.documents += getNonNegativeNumber(cycle.documents);
    bucket.bytes += getNonNegativeNumber(cycle.bytes);
    bucket.duration += duration;
    bucket.maxDuration = Math.max(bucket.maxDuration, duration);
  }

  const configuredWindow = getLiveQueryPollingOption(
    'liveQueryPollingWindowMs',
    LIVE_QUERY_POLLING_DEFAULT_WINDOW_MS
  );
  const windowMs = Math.min(configuredWindow, LIVE_QUERY_POLLING_DEFAULT_WINDOW_MS);
  const cutoff = timestamp - windowMs;

  // A large initial result is not necessarily pathological: catalogIcons can
  // intentionally fetch 12,000 documents once. Re-running that query is the
  // costly pattern, so retain a rolling aggregate per polling observer.
  // Minute buckets and a hard 16-bucket bound avoid per-document
  // instrumentation, extra queries, and unbounded history.
  state.buckets = state.buckets
    .filter(bucket => bucket.start + LIVE_QUERY_POLLING_BUCKET_MS > cutoff)
    .slice(-LIVE_QUERY_POLLING_MAX_BUCKETS);

  const ownerInfo = observerDriver._ownerInfo;
  if (!ownerInfo) {
    return;
  }

  const totals = state.buckets.reduce((result, bucket) => {
    result.polls += bucket.polls;
    result.documents += bucket.documents;
    result.bytes += bucket.bytes;
    result.duration += bucket.duration;
    result.maxDuration = Math.max(result.maxDuration, bucket.maxDuration);
    return result;
  }, {
    polls: 0,
    documents: 0,
  ...
  const minCycles = getLiveQueryPollingOption(
    'liveQueryPollingMinCycles',
    LIVE_QUERY_POLLING_DEFAULT_MIN_CYCLES
  );

  if (totals.polls < minCycles || totals.documents < documentBudget) {
    return;
  }

  const cursorDescription = observerDriver._cursorDescription || {};
  const collection = cursorDescription.collectionName || '<unknown>';
  const publication = this._getPublicationName(ownerInfo.name);
  const reportKey = `${publication}\u0000${collection}`;
  const lastReportedAt = this.liveQueryPollingReports.get(reportKey);

  if (
    lastReportedAt !== undefined &&
    timestamp - lastReportedAt < LIVE_QUERY_POLLING_REPORT_INTERVAL_MS
  ) {
    return;
  }

  this.liveQueryPollingReports.set(reportKey, timestamp);
  this._pruneLiveQueryPollingReports(timestamp);

  const cursorOptions = cursorDescription.options || {};
  const nonOplogReason = getNonOplogReason(cursorDescription, observerDriver);
  const context = {
    publication,
    collection,
    pollingIntervalMs:
      cursorOptions.pollingIntervalMs ||
      cursorOptions._pollingInterval ||
      Number(process.env.METEOR_POLLING_INTERVAL_MS) ||
      10000,
    windowMs,
    pollCount: totals.polls,
    fetchedDocuments: totals.documents,
    approximateFetchedBytes: totals.bytes,
    cumulativePollDurationMs: totals.duration,
    maximumPollDurationMs: totals.maxDuration,
    nonOplogReason,
    cpuUsage: Kadira.models.system?.currentCpuUsage,
  };
  const error = new Error(
    `Repeated live-query polling detected: ${publication} / ${collection}`
  );
  error.stack = `${error.stack}\nLive-query polling context: ${JSON.stringify(context)}`;
...
PubsubModel.prototype._pruneLiveQueryPollingReports = function (timestamp) {
  const staleBefore = timestamp - LIVE_QUERY_POLLING_REPORT_INTERVAL_MS;

  for (const [key, lastReportedAt] of this.liveQueryPollingReports) {
    if (lastReportedAt < staleBefore) {
      this.liveQueryPollingReports.delete(key);
    }
  }
};

function getLiveQueryPollingOption (name, defaultValue) {
  const value = Kadira.options[name];
...
function getNonOplogReason (cursorDescription, observerDriver) {
  try {
    return Kadira.checkWhyNoOplog?.(cursorDescription, observerDriver)?.code ||
      'UNKNOWN_REASON';
  } catch (error) {
    logger('Unable to determine why live query is polling', error);
...
PubsubModel.prototype.trackWaitTime = function (msg) {
  ...
  let timestamp = Ntp._now();
  let metrics = this._getMetrics(timestamp, subscriptionState.publication);

  metrics.waitTime += timestamp - subscriptionState.startTime;
};
