...
setInterval(() => {
  ...
  let status = getServerStatus(primary, true);
  if (status) {
...
function getPoolSize() {
  if (client && client.topology && client.topology.s && client.topology.s.options) {
    // https://www.mongodb.com/community/forums/t/what-is-the-difference-between-poolsize-and-maxpoolsize/122757/2
    // poolSize has been replaced with maxPoolSize in 4.x 
    return client.topology.s.options.maxPoolSize || client.topology.s.options.poolSize || DEFAULT_MAX_POOL_SIZE;
  }
...
Meteor.startup(() => {
  ...
  let majorVersion = parseInt(
    MongoInternals.NpmModules.mongodb.version.split('.')[0],
    10
  );

  if (!options.useUnifiedTopology && majorVersion < 4) {
  ...
  let primaryDescription = getServerDescription(primary);
...
function getPrimary() {
  ...
  // https://github.com/mongodb/node-mongodb-native/blob/00be68fbc3b6b4b27858969dddbb9c595d5da9c6/src/sdam/topology.ts#L797
  // https://github.com/mongodb/node-mongodb-native/blob/main/src/sdam/topology.ts#L797
  // It used to be lastIsMaster until 4.3.1 where they changed it
