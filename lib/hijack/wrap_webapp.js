...
export async function wrapWebApp () {
  ...
  const Layer = WebApp.rawExpressHandlers.parent._router.stack[0].constructor;
  ...
  WebApp.rawExpressHandlers.parent._router.stack.unshift(layer);
  ...
  function wrapHandler (handler) {
    ...
    function wrapper (req, res, next) {
      ...
      if (errorHandler) {
        potentialPromise = handler.call(this, error, req, res, next);
      } else {
        potentialPromise = handler.call(this, req, res, next);
      }
    ...
    if (errorHandler) {
      return function (error, req, res, next) {
        return runWithALS(wrapper(error, req, res, next));
      };
    }
    return function (req, res, next) {
      return runWithALS(wrapper(req, res, next));
    };
  }
  ...
  wrapConnect(WebApp.rawExpressHandlers, false);
  ...
  wrapConnect(WebApp.expressHandlers, true);

  wrapConnect(WebApp.expressApp, false);
