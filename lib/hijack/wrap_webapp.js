...
MAX_STRINGIFIED_BODY_SIZE = 2000
...
export async function wrapWebApp() {
  ...
  WebApp.rawConnectHandlers.stack.unshift({
    ...
    Kadira.tracer.event(trace, 'start', {
      url: req.url,
      method: req.method,
      // TODO: add option for headers that are removed
    ...
    res.on('finish', () => {
      ...
      // set req.body
      // Technically any method can send a body, but since many load balancers and
      ...
      if (req.method === 'POST' && req.body && isJson && hasSmallBody) {
        try {
          // TODO: allow configuring body fields that are scrubbed
          let body = JSON.stringify(req.body);

          // Check for if the body is much larger than
          // what was in the content-length header
