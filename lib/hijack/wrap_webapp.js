...
export function getRouter (app) {
  // Express 5 removed the _ prefix to router.
  return app.parent._router || app.parent.router;
}
...
export function wrapWebApp () {
  ...
  const Layer = getRouter(WebApp.rawHandlers).stack[0].constructor;
  ...
  const middleware = (req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      if (req.method === 'POST' && req.body && isJson && hasSmallBody) {
        try {
          let filteredBody = Kadira.tracer._applyObjectFilters(req.body);
          let body = JSON.stringify(filteredBody);
  ...
  getRouter(WebApp.rawHandlers).stack.unshift(layer);

  function wrapHandler (handler, possibleRouteName) {
    if (Array.isArray(handler)) {
      return handler.map((singleHandler) => {
        return wrapHandler(singleHandler, possibleRouteName);
      });
    }
  ...
  function wrapExpress (app, wrapStack) {
    ...
    app.use = function (...args) {
      let fn = args[args.length - 1] = args[args.length - 1];

      // Make sure this isn't an instance of an express app, since our wrapping
      // would prevent express from detecting if it is
      // https://github.com/monti-apm/monti-apm-agent/issues/133
      // TODO: instrument child express app instances
      if (!fn.handle) {
  ...
  WebAppInternals.staticFilesMiddleware = function (
    ...
    return oldStaticFilesMiddleware.call(
      this,
      _staticFiles,
      req,
      res,
