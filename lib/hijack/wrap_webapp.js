...
import { runWithALS, startTrackingAsync } from '../als/als';
...
export async function wrapWebApp () {
  ...
  /**
   * @todo For some reason the `stack` is undefined after the migration to express, we need to investigate.
   */
  if (!WebApp.rawExpressHandlers.stack) return;

  // Adding the handler directly to the stack
  // to force it to be the first one to run
  WebApp.rawExpressHandlers.stack.unshift({
    route: '',
    handle: (req, res, next) => {
