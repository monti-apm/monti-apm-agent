    ...
    if(msg.msg == 'result') {
      ...
      if(kadiraInfo) {
        if(msg.error) {
          ...
          if (isForced) {
            console.warn('Monti APM endevent forced complete', JSON.stringify(kadiraInfo.trace.events));
          };
  ...
  methodMap[name] = function() {
    try{
      ...
      if(ex && Kadira._getInfo()) {
        ...
        // TODO: find a better way since this causes Meteor._debug to show
        // the stack as an object when error tracking is disabled
