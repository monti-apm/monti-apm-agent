...
export function wrapSession (sessionProto) {
  ...
  sessionProto.processMessage = function (msg) {
    if (true) {
      ...
      if (msg.msg === 'method' || msg.msg === 'sub') {
        ...
        const startData = { userId: this.userId, params: stringifiedParams };
  ...
  sessionProto.protocol_handlers.method = function (msg, unblock) {
    const self = this;
    // add context
    const kadiraInfo = msg.__kadiraInfo;
    let response;
    if (kadiraInfo) {
      Kadira._setInfo(kadiraInfo);

      // end wait event
      const waitList = Kadira.waitTimeBuilder.build(this, msg.id);
  ...
  const orginalSubHandler = sessionProto.protocol_handlers.sub;
  ...
  // to capture the currently processing message
  const orginalUnSubHandler = sessionProto.protocol_handlers.unsub;
  sessionProto.protocol_handlers.unsub = function (msg, unblock) {
    unblock = Kadira.waitTimeBuilder.trackWaitTime(this, msg, unblock);
    const response = orginalUnSubHandler.call(this, msg, unblock);
    unblock();
    return response;
  };
