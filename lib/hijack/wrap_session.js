...
function setupTimeout ({ kadiraInfo, type, method}) {
  if (type !== 'method') {
    return;
  }

  if (!Kadira.options.trackingTimeout) {
    return;
  }

  kadiraInfo.timeoutId = ++timeoutId;

  const error = new Error(`Method Timeout (${Kadira.options.trackingTimeout}ms): ${method}`);

  timeoutMap.set(kadiraInfo.timeoutId, setTimeout(() => {
    ...
    timeoutMap.delete(kadiraInfo.timeoutId);
  }, Kadira.options.trackingTimeout));
}
...
wrapSession = function (sessionProto) {
  ...
  sessionProto.send = function (msg) {
    if (msg.msg == 'result') {
      ...
      if (kadiraInfo) {
        if (kadiraInfo.timeoutId && timeoutMap.has(kadiraInfo.timeoutId)) {
          clearTimeout(timeoutMap.get(kadiraInfo.timeoutId));
          timeoutMap.delete(kadiraInfo.timeoutId);
          kadiraInfo.timeoutId = undefined;
        }
