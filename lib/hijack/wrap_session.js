...
export function wrapSession (sessionProto) {
  ...
  sessionProto.processMessage = function (msg) {
    ...
    if (msg.msg === 'method' || msg.msg === 'sub') {
      ...
      Kadira.tracer.event(kadiraInfo.trace, EventType.Start, startData);
      msg._waitEventId = Kadira.tracer.event(kadiraInfo.trace, EventType.Wait, {}, kadiraInfo);
  ...
  sessionProto.protocol_handlers.method = runWithALS(async function (msg, unblock) {
    let self = this;
    // add context
  ...
  sessionProto.protocol_handlers.sub = runWithALS(async function (msg, unblock) {
    ...
    if (kadiraInfo) {
      ...
      // end wait event
      Kadira.models.pubsub.trackWaitTime(msg);
      let waitList = Kadira.waitTimeBuilder.build(this, msg.id);
      ...
      if (kadiraInfo) {
        ...
        if (msg.error) {
          ...
          if (kadiraInfo && kadiraInfo.currentError) {
            ...
            // see wrapMethodHandlerForErrors() method def for more info
          ...
          Kadira.tracer.event(kadiraInfo.trace, EventType.Error, {error});
        } else {
          Kadira.tracer.endLastEvent(kadiraInfo.trace);
...
Meteor.server.publish_handlers = new Proxy(Meteor.server.publish_handlers, {
  get (target, propKey) {
    ...
    return function (...args) {
      return awaitDetector.detect(() => origMethod.apply(this, args), Kadira._getInfo());
    };
  }
});
...
Meteor.methods = function (methodMap) {
  _.each(methodMap, function (handler, name) {
    wrapMethodHandlerForErrors(name, handler, methodMap);
  });
...
function wrapMethodHandlerForErrors (name, originalHandler, methodMap) {
  methodMap[name] = function () {
    ...
    try {
      let result = awaitDetector.detect(() => originalHandler.apply(this, arguments), Kadira._getInfo());
      if (result && typeof result.then === 'function') {
        // Capture the rejection before Meteor handles it, but return the
        // original promise. Returning the promise created by `then` adds a
        // level to the promise chain that the await detector sees as an
        // additional await covering the whole method.
