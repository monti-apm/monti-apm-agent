...
function shouldExitOnUncaughtException (err) {
  return !err._montiKeepProcessAlive && Kadira.options.exitOnUncaughtException !== false;
}
...
export function TrackUncaughtExceptions () {
  process.on('uncaughtException', function handleUncaughtException (err) {
    ...
    if (!Kadira.options.enableErrorTracking) {
      if (shouldExitOnUncaughtException(err)) {
    ...
    function throwError (_err) {
      ...
      process.nextTick(function () {
        ...
        _err._tracked = true;
        if (shouldExitOnUncaughtException(_err)) {
