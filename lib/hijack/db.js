...
export function hijackDBOps () {
  ...
  [
    'findOne', 'update', 'remove', 'insert', '_ensureIndex', '_dropIndex', 'createIndex'
    ...
    mongoConnectionProto[func] = function (collName, selector, mod, options) {
      ...
      if (func === 'findOne' && mod?.projection) {
        payload.projection = JSON.stringify(mod.projection);
      } else if (func === 'findOne' && mod?.fields) {
