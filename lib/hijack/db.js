...
export function hijackDBOps () {
  ...
  NORMAL_METHODS.forEach(function (func) {
    ...
    console.warn('hijacking normal', func, originalFunc);
  ...
  ASYNC_METHODS.forEach(function (func) {
    ...
    console.warn('hijacking async', func, originalFunc);
    ...
    mongoConnectionProto[func] = async function (collName, selector, mod, options) {
      ...
      try {
        ret = await originalFunc.apply(this, arguments);
  ...
  const CURSOR_METHODS = ['fetch', 'fetchAsync', 'forEachAsync', 'mapAsync', 'countAsync', 'observeChanges', 'observe'];

  CURSOR_METHODS.forEach(function (type) {
    ...
    cursorProto[type] = async function () {
      ...
      try {
        ...
        // `observeChanges` and `observe` might not return a promise.
        ...
        if (['observeChanges', 'observe'].includes(type)) {
          let observerDriver;
          endData.oplog = false;
        ...
        if (['fetch', 'fetchAsync', 'map', 'mapAsync'].includes(type)) {
          // for other cursor operation

          endData.docsFetched = ret.length;

          if (['fetch', 'fetchAsync'].includes(type)) {
