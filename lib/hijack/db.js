...
export function hijackDBOps () {
  ...
  NORMAL_METHODS.forEach(function (func) {
    ...
    mongoConnectionProto[func] = function (collName, selector) {
      let payload = {
        coll: collName,
        func,
      };
      ...
      return Kadira.tracer.asyncEvent(EventType.DB, payload, null, () => originalFunc.apply(this, arguments));
    };
  });


  const ASYNC_METHODS = ['insertAsync', 'removeAsync', 'updateAsync', 'createIndexAsync', 'dropIndexAsync', 'ensureIndex', 'findOneAsync'];
  ...
  ASYNC_METHODS.forEach(function (func) {
    ...
    mongoConnectionProto[func] = async function (collName, selector, mod, options) {
      ...
      return await Kadira.tracer.asyncEvent(EventType.DB, payload, null, async () => {
        ...
        try {
          result = await originalFunc.apply(this, arguments);
          ...
          Kadira.tracer.asyncEventEnd(endData);
        } catch (ex) {
          Kadira.tracer.asyncEventEnd({err: ex.message});
          throw ex;
        }
  ...
  OBSERVE_METHODS.forEach(function (type) {
    ...
    cursorProto[type] = async function () {
      ...
      if (kadiraInfo) {
        eventId = Kadira.tracer.event(kadiraInfo.trace, EventType.DB, payload);
      }
    ...
    cursorProto[type] = async function () {
      ...
      return await Kadira.tracer.asyncEvent(EventType.DB, payload, null, async () => {
        ...
        try {
          ...
          console.log(type === 'fetch', kadiraInfo.activeEvent);
