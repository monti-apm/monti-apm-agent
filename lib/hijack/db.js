...
const ASYNC_METHODS = ['insertAsync', 'findAsync', 'removeAsync', 'updateAsync', 'createIndexAsync', 'dropIndexAsync'];
...
export function hijackDBOps () {
  ...
  [...ASYNC_METHODS, ...NORMAL_METHODS].forEach(function (func) {
    ...
    mongoConnectionProto[func] = function (collName, selector, mod, options) {
      ...
      } else if (['_ensureIndex', '_dropIndex', 'createIndex', 'createIndexAsync', 'dropIndex', 'dropIndexAsync'].includes(func)) {
        // add index
