var client = MongoInternals.defaultRemoteCollectionDriver().mongo.client;
...
setInterval(async () => {
  ...
  let status = getServerStatus(primary);
  if (status) {
...
Meteor.startup(async () => {
  ...
  let primaryDescription = getServerDescription(await getPrimary());
...
async function getPrimary() {
  ...
  let primary = null;
  // https://github.com/meteor/meteor/blob/260a3299ca4c3dd51c302475de4d7a24f055acc2/packages/mongo/mongo_driver.js#L194
  var db = client.db();
  try {
    const helloDocument = db.admin().command({hello: 1}).await();
    // First, figure out what the current primary is, if any.
    if (helloDocument.primary) {
      primary = helloDocument.primary;
    }
  }catch(_){
    ...
    if (isMasterDocument.primary) {
      primary = isMasterDocument.primary;
    }
  }
  return primary;
}
