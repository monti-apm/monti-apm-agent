...
export function wrapOplogObserveDriver (proto) {
  ...
  const originalPublishNewResults = proto._publishNewResults;
  proto._publishNewResults = function (newResults, newBuffer) {
    ...
    const count = newResults.size() + newBuffer.size();
    if (this._ownerInfo) {
  ...
  const originalHandleOplogEntryQuerying = proto._handleOplogEntryQuerying;
  ...
  const originalHandleOplogEntrySteadyOrFetching = proto._handleOplogEntrySteadyOrFetching;
...
export function wrapPollingObserveDriver (proto) {
  const originalPollMongo = proto._pollMongo;
...
export function wrapMultiplexer (proto) {
  const originalInitalAdd = proto.addHandleAndSendInitialAdds;
...
export function wrapForCountingObservers () {
  ...
  const originalObserveChanges = mongoConnectionProto._observeChanges;
  mongoConnectionProto._observeChanges = function (cursorDescription, ordered, callbacks) {
    const ret = originalObserveChanges.call(this, cursorDescription, ordered, callbacks);
    ...
    if (kadiraInfo && ret._multiplexer) {
      if (!ret._multiplexer.__kadiraTracked) {
        ...
        if (kadiraInfo.trace.type === 'sub') {
          ...
          const observerDriver = ret._multiplexer._observeDriver;
