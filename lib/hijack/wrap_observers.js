...
export function wrapPollingObserveDriver (proto) {
  ...
  proto._pollMongo = async function () {
    const startedAt = Date.now();
    const result = await originalPollMongo.call(this);
    ...
    Kadira.models.pubsub.trackLiveQueryPolling(this, {
      documents: count,
      bytes: docSize,
      duration,
    });
...
export function wrapForCountingObservers () {
  ...
  mongoConnectionProto._observeChanges = async function (cursorDescription, ordered, callbacks, options) {
    let ret = await originalObserveChanges.call(this, cursorDescription, ordered, callbacks, options);
    ...
    if (kadiraInfo && ret._multiplexer) {
      if (!ret._multiplexer.__kadiraTracked) {
        ...
        if (kadiraInfo.trace.type === 'sub') {
          ...
          // The initial poll can finish before the publication owner is known.
          Kadira.models.pubsub.trackLiveQueryPolling(ownerStorer);
