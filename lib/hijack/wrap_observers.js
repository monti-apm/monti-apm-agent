...
wrapRedisOplogObserveDriver = function (driver) {
  const driverProto = driver.prototype;
  const MongoColl = typeof Mongo !== "undefined" ? Mongo.Collection : Meteor.Collection;
  const collName = "__dummy_coll_" + Random.id();
  const coll = new MongoColl(collName);
  ...
  observableCollectionProto.add = function (doc, safe) {
    ...
    var docSize = Kadira.docSzCache.getSize(coll, query, opts, [doc]);
    // According to the comments in redis-oplog, "safe" means that the document is "cleaned", whatever that means.
    // But it is used in the initial add and syntetic mutations, so we use it to get initial adds.
    if (this._ownerInfo) {
  ...
  redisSubscriberProto.process = function (op, doc, fields, ...rest) {
  ...
  let originalStop = driverProto.stop;
  driverProto.stop = function () {
