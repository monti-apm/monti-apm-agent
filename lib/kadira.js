...
Kadira.connect = function (appId, appSecret, options) {
  ...
  if (options.liveQueryPollingWindowMs === undefined) {
    options.liveQueryPollingWindowMs = 1000 * 60 * 15;
  }
  if (options.liveQueryPollingMinCycles === undefined) {
    options.liveQueryPollingMinCycles = 3;
  }
  if (options.liveQueryPollingDocumentBudget === undefined) {
    options.liveQueryPollingDocumentBudget = 25000;
  }
...
Kadira._buildPayload = function () {
  ...
  if (
    !Kadira.coreApi ||
    !Kadira.coreApi.featureSupported(Feature.JSON_LINE_TRACES)
  ) {
...
Kadira._sendPayload = function () {
  ...
  let tracePromise;
  let sendPromise = Kadira.coreApi.sendData(payload)
    .then(body => {
      handleApiResponse(body);
    })
    .catch(logErr);
  ...
  if (Kadira.coreApi.featureSupported(Feature.JSON_LINE_TRACES)) {
    ...
    tracePromise = Kadira.coreApi.sendTraces(tracePayload).catch(logErr);
  }

  return Promise.all([sendPromise, tracePromise]);
};
