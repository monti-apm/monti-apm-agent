...
/* global Kadira, __meteor_runtime_config__, MontiProfiler */

import Fibers from 'fibers';
import os from 'os';
import * as MontiApmCore from 'monti-apm-core';
...
const logger = createLogger('kadira:apm');
const KadiraCore = MontiApmCore.Kadira;
...
Kadira.connect = function (appId, appSecret, options) {
  ...
  options.isHostNameSet = Boolean(options.hostname);
  ...
  Meteor.publish(null, function () {
    this.added('kadira_settings', Random.id(), __meteor_runtime_config__.kadira);
    this.ready();
  });
...
Kadira._sendPayload = function () {
  new Fibers(function () {
    const payload = Kadira._buildPayload();
