...
Kadira.connect = function (appId, appSecret, options) {
  ...
  options.trackingTimeout = options.trackingTimeout || 1000 * 60 * 10;
  ...
  Meteor.publish(null, function () {
    let options = __meteor_runtime_config__.kadira;
...
Kadira.trackError = function () {
  ...
  const { message, subType, stack, type, kadiraInfo = Kadira._getInfo() } = getErrorParameters(arguments);

  const previousEvents = kadiraInfo && kadiraInfo.trace && [...kadiraInfo.trace.events] || [{ type: 'start', at: 0, endAt: 0 }];

  const events = Kadira.tracer.optimizeEvents(previousEvents).concat([
    ['error', 0, { error: { message, stack } }, { forcedEnd: true}]
  ]);
