...
Kadira.connect = function (appId, appSecret, options) {
  ...
  if (options.disableAwaitDetector) {
    console.log('Monti APM: Disabling await detector');
    
    awaitDetector.unregister()
    awaitDetector.detect = (fn)=> fn()
  }
...
Kadira.traceJob = function (details, processor) {
  ...
  try {
    let self = this;
    let args = arguments;
    result = awaitDetector.detect(() => processor.apply(self, args));
