...
import { MontiAsyncStorage, MontiEnvironmentVariable } from './als';
...
Kadira._getInfo = function (useEnvironmentVariable) {
  if (useEnvironmentVariable) {
    // console.log(Kadira.env.kadiraInfo.get(), new Error().stack);

    return MontiEnvironmentVariable.getStore();
  }

  // prettyLog(`Getting Info ${new Error().stack.split('\n')[2]?.trim()}`, MontiAsyncStorage.getStore());

  return MontiAsyncStorage.getStore()?.get('info');
};

// this does not clone the info object. So, use with care
Kadira._setInfo = function (info) {
  // prettyLog('Setting Info', { info });

  return MontiAsyncStorage.getStore().set('info', info);
};
