...
import { handleJobAdded, jobExecutor } from './jobs';
...
const { CoreEvent } = Npm.require('monti-apm-core');
  ...
  if (appId && appSecret) {
    ...
    Kadira.coreApi._checkAuth()
      .then(function () {
        ...
        jobExecutor(Kadira.coreApi);

        Kadira.coreApi.on(CoreEvent.JOB_ADDED, Meteor.bindEnvironment(job => {
          handleJobAdded(job);
        }));
      })
      .catch(function (err) {
