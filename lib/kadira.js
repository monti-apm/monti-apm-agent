...
Kadira.connect = function (appId, appSecret, options) {
  ...
  if (appId && appSecret) {
    ...
    Kadira.coreApi = new KadiraCore({
      ...
      agentVersion: packageVersions['montiapm:agent'] || '<unknown>'
    });
...
Kadira._sendAppStats = function () {
  ...
  _.each(Package, function (v, name) {
    appStats.packageVersions.push({
      name,
      version: packageVersions[name] || null
    });
  });
...
Kadira.event = function (name, fn, data = {}) {
  let info = getInfo();

  if (typeof fn !== 'function') {
    throw new Error('Monti APM: Kadira.event requires a function');
  }

  if (info) {
    const event = Kadira.tracer.event(info.trace, EventType.Custom, data, { name });
    return Kadira.tracer.runWithEvent(fn, event);
  }
