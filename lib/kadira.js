...
Kadira._buildPayloads = function () {
  let basePayload = { host: Kadira.options.hostname, clientVersions: getClientVersions() };

  // shallow clone is acceptable since the existing properties are not mutated
  let metricsPayload = Object.assign({}, basePayload);
  let tracesPayload = Object.assign({}, basePayload);

  metricsPayload.methodMetrics = Kadira.models.methods.buildMetricsPayload();
  metricsPayload.pubsubMetrics = Kadira.models.pubsub.buildMetricsPayload();
  metricsPayload.systemMetrics = Kadira.models.system.buildMetricsPayload();
  metricsPayload.jobsMetrics = Kadira.models.jobs.buildMetricsPayload();
  metricsPayload.httpMetrics = Kadira.models.http.buildMetricsPayload();

  if (Kadira.options.enableErrorTracking) {
    metricsPayload.errors = Kadira.models.errors.buildPayload();
  }

  tracesPayload.methodRequests = Kadira.models.methods.buildTracesPayload();
  tracesPayload.pubRequests = Kadira.models.pubsub.buildTracesPayload();
  tracesPayload.jobRequests = Kadira.models.jobs.buildTracesPayload();
  tracesPayload.httpRequests = Kadira.models.http.buildTracesPayload();
...
Kadira._sendPayload = function () {
  new Fibers(function () {
    let { metricsPayload, tracesPayload } = Kadira._buildPayloads();
...
Kadira._traceJob = function (details, processor) {
  let info = Kadira._getInfo();
  if (info || !details) {
  ...
  function end (err) {
    ...
    if (err) {
      let error = _.pick(err, ['message', 'stack', 'details']);
      Kadira.tracer.event(trace, 'error', { error });
    } else {
