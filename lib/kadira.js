...
Kadira.trackError = function () {
  ...
  if (
    ...
    type = arguments[0] || 'server-internal';
    message = arguments[1];
    subType = arguments[2] && arguments[2].subType || 'server'; 
    ...
    stack = error.stack || '';
    type = options && options.type || 'server-internal';
    subType = options && options.subType || 'server';
  }

  if (message && subType) {
    var error = {message, stack};
    var trace = {
      type,
      subType,
      name: message,
