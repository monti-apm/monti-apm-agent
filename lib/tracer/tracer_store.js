...
const logger = createLogger('kadira:ts');

/**
 * @param {object } [options]
 * @param {number} [options.maxTotalPoints=30]
 * @param {number} [options.interval=60000]
 * @param {number} [options.archiveEvery]
 * @constructor
 */
...
TracerStore.prototype.processTraces = function () {
  ...
  for (let kind of kinds) {
    ...
    const currentMaxTrace = self.currentMaxTrace[kind];
    ...
    const archiveDefault = (self.processedCnt[kind] % self.archiveEvery) === 0;
