...
Tracer.prototype.optimizeEvents = function (events, metrics) {
  ...
  return events
    .filter(e => e.type !== EventType.Async || e.duration > 0 || e.nested?.length)
    .slice(0, MAX_TRACE_EVENTS).map(event => this.optimizeEvent(event, metrics));
};
