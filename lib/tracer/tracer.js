...
Tracer.prototype.optimizeEvent = function (objectEvent, metrics) {
  ...
  const optimizedEvent = [type];

  if (typeof duration === 'number') {
    if (metrics) {
      metrics.work += duration;

      if (duration > 0 && ![EventType.Complete, EventType.Start].includes(type)) {
        metrics[type] = metrics[type] || 0;
        metrics[type] += duration;
      }
    }
    optimizedEvent.push(duration);
  }

  if (objectHasData(data)) {
    if (typeof duration !== 'number') {
      optimizedEvent.push(null);
    }

    optimizedEvent.push(data);
  }
...
Tracer.prototype.optimizeEvents = function (events, metrics) {
  ...
  events.slice(0, MAX_TRACE_EVENTS).forEach((event) => {
    if (prevEvent.endAt && event.at) {
      const computeTime = event.at - prevEvent.endAt;

      if (computeTime > 0) {
        optimizedEvents.push([EventType.Compute, computeTime]);
      }
    }

    optimizedEvents.push(this.optimizeEvent(event, metrics));
