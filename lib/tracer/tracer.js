...
import { getActiveEvent, getInfo, getStore, MontiAsyncStorage } from '../als/als';
...
export const checkForNested = event => {
  if (!event.nested) {
    ...
    console.dir(getInfo()?.trace, { depth: 10 });
  }
};
...
Tracer.prototype.buildTrace = function (traceInfo) {
  ...
  metrics.asyncIntervalsMerged = mergeOverlappingIntervals(metrics.asyncIntervals);

  console.trace();
  prettyLog('Work Intervals', metrics.workIntervalsMerged);
  prettyLog('Async Intervals', metrics.asyncIntervalsMerged);
...
Tracer.prototype.optimizeEvent = function (objectEvent, metrics) {
  ...
  let {at, endAt, stack, nested = [], name, forcedEnd, type, data, asyncId, level} = objectEvent;
  ...
  if (![EventType.Complete, EventType.Start].includes(type)) {
    ...
    if (isNumber(duration)) {
      if (metrics) {
        if (type !== EventType.Async && level === 0) {
