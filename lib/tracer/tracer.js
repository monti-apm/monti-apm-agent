...
import { getActiveEvent, setActiveEvent } from '../als/als';
...
Tracer.prototype.event = function (traceInfo, type, data, metaData) {
  ...
  if (activeEvent && !activeEvent.endAt) {
    ...
    // By returning here, we prevent setting the active event for nested events, thus not going deeper than one level.
  ...
  if (![EventType.Error, EventType.Start, EventType.Complete].includes(type)) {
