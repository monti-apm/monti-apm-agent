...
Tracer.prototype._hasUsefulNested = function (event) {
  return !event.nested.every(event => event.type === 'async');
};
...
Tracer.prototype.optimizeEvent = function (objectEvent) {
  const {at, endAt, stack, nested = [], forcedEnd, name} = objectEvent;

  const computeTime = at && endAt ? endAt - at : 0;

  const optimizedNestedEvents = nested.length && this._hasUsefulNested(objectEvent) ? this.optimizeEvents(nested) : undefined;

  const optimizedEvent = [objectEvent.type, computeTime, objectEvent.data || {}];
