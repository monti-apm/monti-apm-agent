...
import { getTotalDuration, mergeParallelIntervalsArray, setEventDuration, subtractIntervals } from '../utils/time';
...
Tracer.prototype.event = function (trace, type, data, meta) {
  ...
  if (data?.shouldBeSibling) {
    activeEvent = activeEvent?.parentEvent || null;
  }
...
Tracer.prototype.asyncEvent = function (type, data, meta, fn) {
  ...
  if (util.types.isAsyncFunction(fn)) {
    callback = async () => {
      try {
        let result = await Reflect.apply(fn, this, [event]);
  ...
  return this.runWithEvent(callback, event);
};
...
Tracer.prototype.buildTrace = function (traceInfo) {
  ...
  // prettyLog('events', traceInfo.events.filter(e => e.type !== EventType.Async || e.duration > 0 || e.nested?.length));
...
Tracer.prototype.optimizeEvent = function (objectEvent, metrics) {
  ...
  cleanTrailingNilValues(optimizedEvent);
...
Tracer.prototype.optimizeEvents = function (events, metrics) {
  ...
  return events
    .map(event => this.optimizeEvent(event, metrics))
    // We truncate the trace after all metrics have been calculated
