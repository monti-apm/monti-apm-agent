...
let TRACE_TYPES = ['sub', 'method', 'http', 'job'];
...
Tracer.prototype.asyncEvent = function (type, data, fn) {
  const { info } = getStore();
...
Tracer.prototype.optimizeEvent = function (objectEvent, prevEnded, nestedMetrics) {
  ...
  if (![EventType.Complete, EventType.Start].includes(type)) {
    if (!endAt) {
      const info = getInfo();
      // In some tests, we don't have a trace
      endAt = info?.trace?.endAt || Ntp._now();
