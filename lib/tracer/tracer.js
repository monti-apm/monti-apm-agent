...
Tracer.prototype.asyncEvent = function (type, data, meta, fn) {
  const { info, activeEvent = null } = getStore();

  if (!info) {
    return Reflect.apply(fn, this, [activeEvent]);
  }
  ...
  // If we already have an active event, we use it as parent to prevent nesting more than one level deep.
  }, activeEvent ?? event);
};
