...
Tracer.prototype.start = function (name, type, {
  ...
  if (Kadira.options.traceTimeout) {
    traceInfo.timeoutId = Meteor.setTimeout(() => {
      const error = new Error(`Method Timeout (${Kadira.options.trackingTimeout}s): ${type.name}`);
      Kadira.models.error.trackError(error, traceInfo);
