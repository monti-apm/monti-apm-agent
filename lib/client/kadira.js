...
Kadira.webVitals = new WebVitalsModel();
...
function initialize (options = {}) {
  ...
  if (Kadira.options.appId && Kadira.options.endpoint) {
    ...
    if (Kadira.options.enableClientPerformanceTracking) {
      // eslint-disable-next-line no-console
      console.log('MontiAPM: Client Performance Tracking is enabled');
      Kadira.webVitals.startTracking();
    }
...
Kadira.trackRouteTransition = ({ route }) => {
  Kadira.currentRoute = route;
};
