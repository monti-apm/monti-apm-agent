...
export class WebVitalsModel {
  ...
  stopTracking () {
    this.meteorStartupTime = Ntp._now();
    unwrapDynamicImport();
  ...
  startTracking () {
    ...
    Tracker.autorun((computation) => {
      ...
      if (connected) {
        this.sentPingAt = Ntp._now();
        Meteor.connection._send({msg: 'ping', id: 'montiapm'});
    ...
    Meteor.startup(() => {
      Meteor.connection._stream.on('message', (rawMsg) => {
        if (this.receivedPongAt) return;
        const parsedMessage = DDPCommon.parseDDP(rawMsg);
        if (parsedMessage.msg === 'pong' && parsedMessage.id === 'montiapm') {
          this.receivedPongAt = Ntp._now();
        }
      });
  ...
  _buildPayload = function (metricsArray) {
    // startTime is used for parititioning
    const metrics = {startTime: this.startTime, pageLoadTime: this.meteorStartupTime - this.startTime};
    ...
    // TODO: have a way to get initial connection attempt - in the meanwhile we can use resourcesResponseEnd
    // metrics.connectionTimeStart = metrics.resourcesResponseEnd;
    metrics.connectionTimeEnd = this.connectionTime - this.startTime;
    if (metrics.loginTimeEnd) {
    ...
    const payload = {
      ...
      isUsingSockJS: Meteor.connection._stream.socket.constructor.name !== 'WebSocket',
      sockjsTransport: Meteor.connection._stream.socket.transport,
      sockJSDelay: this.receivedPongAt - this.sentPingAt,
