...
| liveQueryPollingWindowMs | OPTIONS_LIVE_QUERY_POLLING_WINDOW_MS | 900000 (15m) | Rolling window for detecting repeated Mongo live-query polling. Values above 15 minutes are capped to keep observer state bounded. |
| liveQueryPollingMinCycles | OPTIONS_LIVE_QUERY_POLLING_MIN_CYCLES | 3 | Minimum polls in the rolling window before reporting the diagnostic. |
| liveQueryPollingDocumentBudget | OPTIONS_LIVE_QUERY_POLLING_DOCUMENT_BUDGET | 25000 | Minimum cumulative fetched documents in the rolling window. Set to `0` to disable the diagnostic. |
