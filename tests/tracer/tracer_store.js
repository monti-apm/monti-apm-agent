...
Tinytest.add(
  ...
  function (test) {
    ...
    const mean = ts._getMean([10, 20, 30]);
    test.equal(mean, 20);
  }
);
...
Tinytest.add(
  ...
  function (test) {
    const ts = new TracerStore();
    const median = ts._getMedian([10, 20, 30, 40, 1, 3]);
...
Tinytest.add(
  ...
  function (test) {
    const ts = new TracerStore();
    const dataSet = [10, 20, 30, 40, 1, 3, 3];
    const median = ts._getMedian(dataSet);
    const mad = ts._calculateMad(dataSet, median);
    test.equal(mad, 9);
  }
);
...
Tinytest.add(
  ...
  function (test) {
    const ts = new TracerStore();
    const dataSet = [10, 20, 30, 40, 1, 3, 3];
    const isOutlier = ts._isOutlier(dataSet, 40, 3);
...
Tinytest.add(
  ...
  function (test) {
    const ts = new TracerStore();
    const dataSet = [10, 20, 30, 40, 20, 30, 30, 1];
    const isOutlier = ts._isOutlier(dataSet, 1, 3);
    test.equal(isOutlier, true);
  }
);
...
Tinytest.add(
  ...
  function (test) {
    ...
    const currentMaxTrace = ts.currentMaxTrace;
...
Tinytest.add(
  ...
  function (test) {
    const ts = new TracerStore({archiveEvery: 3});
    ts.addTrace({name: 'one', type: 'method', metrics: {total: 100}});
    ts.processTraces();
...
Tinytest.add(
  ...
  function (test) {
    const ts = new TracerStore({maxTotalPoints: 3});
    ts.addTrace({name: 'one', type: 'method', metrics: {total: 100}});
    ts.processTraces();
    ts.processTraces();
...
Tinytest.add(
  ...
  function (test) {
    const ts = new TracerStore({archiveEvery: 20});
    ts.addTrace({name: 'one', type: 'method', metrics: {total: 100}});
    ts.processTraces();
